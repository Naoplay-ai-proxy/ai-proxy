...
class SpyLLMClient:
    ...
    # Capture les arguments envoyés au LLM pour vérifier comment l'application 
    # transforme l'entrée utilisateur avant l'envoi (Sandboxing).
...
def test_router_sandboxing(spy_client):
    # Vérifie la stratégie de "Sandboxing" : même si un input passe la validation regex,
    # il doit être encapsulé entre des balises strictes (TRANSCRIPT START/END) pour être 
    # traité comme de la donnée brute et non comme une instruction par le LLM.
